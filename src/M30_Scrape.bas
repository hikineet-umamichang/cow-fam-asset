...
'            Debug.Print "  個体情報 行数: " & IIf(IsEmpty(kotai), 0, UBound(kotai, 1))
'            Debug.Print "  異動情報 行数: " & IIf(IsEmpty(idou), 0, UBound(idou, 1))
'        Else
'            Debug.Print "[" & i & "/" & total & "] " & cowID & " ... 取得失敗"
'        End If

        ' 必要なら少し待機（サーバへの負荷を下げる）
